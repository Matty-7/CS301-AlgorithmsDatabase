...
class MAX_HEAP:
    ...
    def MAX_HEAPIFY(self, i):   # use self.heapsize as the heapsize
        ...
        if max!=i:
            ...
            self.A[i]=self.A[max]
            self.A[max]=temp
            print(f'POST MAX_HEAPIFY({i}): {self.A}')
            print()
            print(rIO.concat_text_block(before, self.DrawTree(i), 'MAX_HEAPIFY(%s)[LOCAL_FIX]\nheapsize=%s'%(i, self.heapsize)))
            self.MAX_HEAPIFY(max)
        else:
            print(f'POST MAX_HEAPIFY({i}): {self.A}')
            print()
            print(rIO.concat_text_block(before, self.DrawTree(i), 'MAX_HEAPIFY(%s)[same]\nheapsize=%s'%(i, self.heapsize)))
