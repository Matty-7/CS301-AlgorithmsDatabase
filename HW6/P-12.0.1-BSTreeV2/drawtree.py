...
#
# Adapted by Jiang Long for CS301 Fall 2021
#
#    2021.10 DKU
# 
#
...
def deserialize(string):
    if string == '{}':
        return None
    nodes = [None if val == '#' else TreeNode((val))
...
# lj-begin
def draw_level_order2(sx):
    f = io.StringIO()
    with redirect_stdout(f):
        draw_level_order(str([i for i in sx]))
        pass
    s = f.getvalue()
    return s

def draw_level_order3(ss):
    """ The serialization of a binary tree follows a level order traversal,
    where '#' signifies a path terminator where no node exists below.
